...
3. 用户在应用内手动切换到其他会话。后续 Bridge 消息路由到该会话（FEAT-041 的现有行为）；ChatScreen 保持在该会话上。
...
5. 用户在应用中手动切换会话的操作不受影响。
6. 所有现有的 Layer 1A 单元测试继续通过。

### 超出范围

- 反向同步：当用户在应用中切换会话时通知 Bridge（已由 FEAT-041 中的 `getMostRecentSessionId()` 处理）。
- 每个频道对应不同应用会话的多会话映射。
