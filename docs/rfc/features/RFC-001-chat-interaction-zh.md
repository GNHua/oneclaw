...
### 背景
对话交互是 OneClaw 的核心用户面向功能。它编排完整的对话循环：用户发送消息，应用解析当前 Agent 的配置，通过流式 API 将消息发送给 AI 模型，实时渲染响应，处理工具调用（在本地执行工具，将结果发回模型），并重复此过程直到模型产生最终文本响应。本 RFC 涵盖完整的聊天流程，包括流式 SSE 解析、工具调用循环、消息持久化、Markdown 渲染、思考块显示、Agent 切换、停止生成、复制、重新生成，以及聊天界面布局与导航抽屉（RFC-005）和 Agent 选择器（RFC-002）的集成。
