...
OneClawShadow 目前需要用户导航到各个设置界面来配置 provider、模型、agent、主题、工具状态和环境变量。应用已经拥有用于创建 agent（`create_agent`）和管理定时任务（RFC-027）的工具，但缺少管理其余配置的工具。这意味着用户必须在聊天界面和设置 UI 之间频繁切换来调整配置。
